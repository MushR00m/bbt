...
public class WSUtils {
	...
	public static File getResponseAsFile(final String url,final String fileName) {
				...
				.map(new Function<WSResponse, File>() {
					public File apply(WSResponse response) throws Throwable {
						...
						try {
							...
							final File file = new File("E:/data/"+fileName);
	...
	public static String postByXML(final String url, final String xml) {
		// 发送请求
		Promise<WSResponse> jsonPromise = WS
				.url(url)
				.setContentType(
		...
		return jsonPromise.get(100000).toString();
	}
